...
### 2026-06-07 (late) — P5 + full cleanup backlog complete (PRs #6/#8/#9) ✅

Finished the remediation. **P5 core** (#6 `1ad13e7`): SSRF at the fetch boundary (H6/M10, runtime-tested), dead-man's switch (H8), `/api/health` feed probe (H9), `notifyAdmin` critical (M12). **P5 cleanup A+B** (#8 `8c9f16f`): M9 (CF-proxy SSRF guard — private-IP reject + manual redirects), M11 (Inngest `onFailure` on all 5 key functions → admin+Sentry), M4 (digest mass-breakage spike-gate + active count), M5 (broken-list COALESCEs first_broken_at→last_run_at), L3 (dead `broken` pill), L8 (RUNBOOK drift). **P5 cleanup C+D+E** (#9 `ec0618d`): M8 (per-page `requireAdmin()` ×11), M17 (shul_candidate review_status CHECK, migration 0014 applied to prod), deleted orphaned Jina/Docling, H7 (discovery one-shot), E-D2 (cost-gate counts cron spend, gated on `built_at`), bulk re-extract admin button (confirm + the P0 rate gate). **~6 adversarial-review workflows** gated the work — caught + fixed real bugs (worker `fc`/`fd` false-positive, `onFailure` wrong runId, cost-gate over-count). Every deploy smoke-checked green. Deliberately KEPT (working features, not dead code): `is_manual_edit`, PDF tier. Deferred: M16 (build-env risk). The 41→9 outage is fully resolved: recovered, correct, hardened, monitored, cleaned up.
